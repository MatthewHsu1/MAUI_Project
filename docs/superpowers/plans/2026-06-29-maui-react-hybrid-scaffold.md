...
- **Host-platform note:** This dev machine is Linux **with no JDK and no Android SDK installed**, and no macOS/Windows host. Consequence: the platform-neutral projects (Domain, Application, Infrastructure + their tests) and the React app build and test fully here, but **no MAUI target framework can compile to completion on this machine** (android needs JDK + Android SDK; iOS/Mac Catalyst need macOS; the Windows target needs Windows). Per user decision, MAUI target builds are deferred to a configured machine.
- **MAUI-task verification protocol (Tasks 5, 6, 8, 9):** Do NOT expect a successful MAUI target build here. Verify instead by: (1) `dotnet workload install maui` succeeds; (2) `dotnet restore src/AppName.Maui/AppName.Maui.csproj` succeeds (proves project + package + project references resolve); (3) a build attempt `dotnet build src/AppName.Maui/AppName.Maui.csproj -f net10.0-android` is allowed to FAIL, but ONLY at the missing-Android-SDK/JDK step — it must NOT fail with C# compile errors (`CS####`), missing-type, or unresolved-reference errors. If you see `CS####` or reference errors, that is a real defect to fix; failures mentioning Android SDK / `JAVA_HOME` / `aapt` / `Xamarin.Android` tooling are the expected environmental stop and count as "C# verified." Report the exact failure reason so the reviewer can confirm it is environmental.
...
- [ ] **Step 6: Verify per the MAUI-task verification protocol (see Global Constraints)**

Run: `dotnet restore src/AppName.Maui/AppName.Maui.csproj`
Expected: restore succeeds (project, package, and project references resolve).

Then run: `dotnet build src/AppName.Maui/AppName.Maui.csproj -f net10.0-android`
Expected: build attempt fails ONLY at the missing Android SDK / JDK step (e.g. message about Android SDK, `JAVA_HOME`, or Xamarin.Android tooling). It must NOT fail with `CS####` compile errors or unresolved references — those indicate real defects. Capture and report the exact failure reason.
...
- [ ] **Step 4: Verify the bridge compiles (MAUI-task verification protocol)**

Run: `dotnet build src/AppName.Maui/AppName.Maui.csproj -f net10.0-android`
Expected: per the protocol in Global Constraints — failure ONLY at the missing Android SDK / JDK step, with NO `CS####` compile errors (which would mean the bridge code is wrong). Report the exact failure reason.
...
- [ ] **Step 2: Verify the target runs on build (MAUI-task verification protocol)**

Run: `dotnet build src/AppName.Maui/AppName.Maui.csproj -f net10.0-android`
Expected: log shows "Building React app in .../web" and the npm build runs (proving the target fires before the build), then the build reaches the expected environmental stop at the missing Android SDK / JDK step (no `CS####` errors). Confirm `src/AppName.Maui/Resources/Raw/web/index.html` was (re)generated by the React build. Report the exact failure reason.
...
Expected: log does NOT show "Building React app" (the npm build is skipped); the build then reaches the same expected environmental Android-SDK/JDK stop. Report the exact failure reason.
...
- [ ] **Step 4: Full verification — MAUI restore + C# compile check (MAUI-task verification protocol)**

Run: `dotnet restore src/AppName.Maui/AppName.Maui.csproj`
Expected: restore succeeds.

Then run: `dotnet build src/AppName.Maui/AppName.Maui.csproj -f net10.0-android`
Expected: per protocol — failure ONLY at the missing Android SDK / JDK step, no `CS####` errors. This confirms all C# across the MAUI host is consistent. Note in the README/report that completing an actual MAUI target build requires a configured Android/macOS/Windows host.
